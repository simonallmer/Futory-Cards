...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const simulationMap = {
        ...
        'Voider': {
            phase: 0,
            // Both seats get a History Pile worth a real decision: a mix of dead FireSteam
            // and cards actually worth keeping, so declining, voiding 1 and voiding 2 are all
            // sensible plays rather than one obvious answer. Deep enough (6) that the End
            // Phase reshuffle doesn't leave Player 1's Future empty and flip a second event.
            desc: "Player 1's Future Pile is empty — the turn start reveals Voider. Each player may send up to 2 cards from their History Pile into the Abyss, for good. Pick a third card and the unchosen tiles should be dimmed and unclickable. Voided cards leave the History Pile and show up in the shared Abyss (click it to list them).",
            destiny: 'Voider',
            hand: ['FireSteam', 'GoldSteam'],
            p1future: [],
            p1history: ['FireSteam', 'FireSteam', 'FireSteam', 'GoldSteam', 'Ichor', 'Smoke'],
            p2hand: ['FireSteam'],
            p2history: ['FireSteam', 'FireSteam', 'GoldSteam', 'Cravus'],
        },
        'Space Voider': {
            phase: 0,
            // Eight cards deep, because 4 of 6 barely differs from 2 of 6 — a pile this size is
            // where the wider cap actually changes the decision. The mix is deliberate: five
            // cards worth voiding and three worth keeping, so "void the maximum" is NOT the
            // automatic answer. The Computer's History is stocked past its 4-Steam floor so it
            // has a real call to make too.
            desc: "Player 1's Future Pile is empty — the turn start reveals Space Voider: Voider at double depth. Each player may send up to 4 cards from their History Pile into the Abyss, for good. Select four and the rest of the tiles should dim; the Computer voids Steam only, never its Creatures or Artifacts.",
            destiny: 'Space Voider',
            hand: ['FireSteam', 'GoldSteam'],
            p1future: [],
            p1history: ['FireSteam', 'FireSteam', 'FireSteam', 'FireSteam', 'GoldSteam', 'Ichor', 'Smoke', 'Pandorama'],
            p2hand: ['FireSteam'],
            p2history: ['FireSteam', 'FireSteam', 'FireSteam', 'GoldSteam', 'Cravus', 'Reflector'],
        },
        'Eternal Hour': {
            phase: 0,
            // The revealer is deliberately BEHIND: Player 1 on 18, the Computer on 24. Flipping
            // it should knock the LEADER down, not the person who flipped it — that impartiality
            // is the whole card, and it reads far better from this side than from the front.
            day: 6,
            night: 12,
            p2day: 12,
            p2night: 12,
            desc: "Player 1's Future Pile is empty — the turn start reveals Eternal Hour. You are BEHIND (18 TP to the Computer's 24), and the card hits whoever leads — the Computer should drop 24 → 20 while your dice are untouched. Level the two clocks and re-run: with both tied for most, BOTH lose 4.",
    ...
    const devLogData = [
        { date: '2026-08-29', msg: "Destiny 032 Eternal Hour — implemented (auto), 9/24. 'All Players tied for most Time Points lose 4.' Two firsts in one card: it is the first Destiny card with NO choice anywhere in it — nobody picks anything, it just resolves — and the first that is completely IMPARTIAL. Nothing about it keys off the revealer. Flip it while behind and it knocks the leader down for you; flip it while ahead and it hits you and nobody else. That asymmetry IS the card: it is the game's catch-up mechanism, not a weapon, and the sim preset deliberately puts Player 1 BEHIND (18 to the Computer's 24) because the rule reads far better from that side than from the front. THE ONE THING THAT MATTERS IN THE CODE: the tied set is snapshotted BEFORE any damage lands. Resolving seat by seat would let the first hit drop that player out of the lead and change who is 'tied for most' for everyone after them — so a level 24/24 board would punish only the first seat instead of both, which is exactly backwards from what the card says. 'All Players tied for most' is judged once, against the board as the card found it, then applied. THE TIME POINT SYSTEM IS UNTOUCHED: this calls the same resolveDamageDirectly that every attack in the game calls, so the active-die-first rule, the permanent loss of a drained die and the game-over check all come along for free — no new TP path, which is the same discipline the first Destiny commit set after the duplicate-gainTimePoints near-miss. The only care needed was reading each seat's die element BEFORE the hit, since draining the active die changes which one activeDieSel returns and the float would land on the wrong counter. JUDGMENT CALLS. (1) Only players still IN the game are considered — a seat on 0 Time Points is out, and a corpse can't be tied for the lead. In 2-player V1 this can't arise (the game ends the moment a seat hits 0) but the rule is written as it should read at 4 players. (2) It CAN end the game, and if both leaders are on 4 or less it ends in a genuine DRAW; checkGameOver already handles that, so the effect returns early instead of awaiting its notice — otherwise the CONTINUE button would sit trapped behind the game-over screen waiting for a click nobody can give it. (3) Grammar agrees with the subject count: one leader 'leads … and loses 4', several 'lead … and lose 4', and a fully level board gets its own line ('Every player is level on N Time Points'), which is only used when there is more than one survivor. Verified live in BOTH modes across four board states. Vs Computer: (a) the shipped preset — Player 1 on 18, Computer on 24 — hit the COMPUTER only, 24 → 20, with Player 1's dice untouched and the feed reading 'Eternal Hour: The Computer loses 4 Time Points'; (b) a level 24/24 board hit BOTH, each 12/12 → 8/12, proving the snapshot (seat-by-seat resolution would have spared the second); (c) Player 1 on 4 against a Computer on 24 hit the Computer only and left the 4 alone — being nearly dead is not being tied for most; (d) both seats on 4 were both reduced to 0 and the game correctly ended on the DRAW! screen with the Destiny overlay already closed and no orphaned CONTINUE. Hot seat: same result with 'Player 2 leads on 24 Time Points and loses 4', no pass-device screen (nobody chooses anything, so there is nothing to hand over) and the turn resuming at the Steam Phase. No console errors. Destiny progress panel now reads 9/24, next 033 Great Flood." },
        { date: '2026-08-29', msg: "Destiny 031 Space Voider — implemented (auto), 8/24. 'Each Player may send up to 4 Cards from their History Pile into the Abyss.' This is the card 030 was built for: Voider at double depth and nothing else. Because Voider was written as runVoider(max) rather than as one card's worth of logic, the whole of Space Voider is a resolveSpaceVoider that calls it with 4, a row in destinyEffects, a name in implementedCards, an intent badge and a sim preset — no new rules code at all. The clockwise walk, the capped picker, the pile repaint, the Abyss handoff and the Computer's entire heuristic are the same instances, so the two cards cannot drift apart, and 'up to 4' is now genuinely a number rather than a second implementation. ONE FIX the second card exposed: the Computer's feed line was hard-coded to say 'Voider', so a Space Voider reveal logged the wrong event name. runVoider now takes cardName purely for that log — the player-facing overlay text was already card-agnostic and needed nothing. Worth noting the cap CLAMPS to the pile (cap = min(max, history.length)), so on a thin History Space Voider plays exactly like Voider and says so — 'choose up to 2' when there are only 2 cards, not a dead promise of 4. The extra reach only shows once a deck has cycled a few times, which is precisely when thinning is worth the most, and that is the whole design difference between the two cards. Sim preset is deliberately EIGHT cards deep, because 4 of 6 barely differs from 2 of 6 — a pile that size is where the wider cap actually changes the decision — and mixes five cards worth voiding with three worth keeping (Ichor, Smoke, Pandorama) so 'void the maximum' is not the automatic answer. Verified live in BOTH modes. Vs Computer: the reveal read 'choose up to 4', selecting four dimmed the remaining four tiles and a FIFTH click was correctly ignored (still 4 selected), VOID 4 removed exactly the chosen indices (History 8 → 4, survivors intact and in order) and all four landed in the Abyss; the Computer then voided the 3 FireSteam it had and kept its GoldSteam, Cravus and Reflector — Steam only, and it stopped at 3 rather than 4 because that was every FireSteam in its History, not because of the cap. Hard behaved the same with no LaserSteam owned, which is correct: the Gold clause needs a Laser. CLAMP verified by cutting the pile to 2 before the reveal — the prompt read 'up to 2', voiding both emptied the slot cleanly (slot-empty, 'History' label restored). Hot seat: each seat saw only its OWN History at its own cap, the device passed (PASS DEVICE / MY CHOICE) and handed back to the revealer (RESUME TURN) with the turn resuming at the Steam Phase. Feed now names the right card in both directions — 'Voider: sends…' for 030 and 'Space Voider: sends…' for 031. No console errors. Destiny progress panel now reads 8/24, next 032 Eternal Hour." },
        { date: '2026-08-29', msg: "Destiny 030 Voider — implemented (auto), 7/24. 'Each Player may send up to 2 Cards from their History Pile into the Abyss.' The first Destiny card that is pure DECK-BUILDING rather than combat or tempo: the History Pile folds back into the Future when it runs dry, so a card sent to the Abyss leaves that player's deck for good and every remaining draw gets better. Clockwise from the revealer via forEachPlayerClockwise, and the 'may' is real — a seat can void 2, 1 or nothing. Written as runVoider(max) because 031 Space Voider is literally this card with 4 instead of 2; when it comes up it should be one table row and one line, not a second copy of the whole thing. TWO EXTRACTIONS rather than two new copies of existing code, following the takeCardToHand precedent. (1) destinyPickCards only knew how to select over DOM SLOTS (Healing Tree picks Hand cards), and Voider selects over plain card records INSIDE a pile. Rather than a second picker, the core became destinyPickTiles(items, opts) taking { card, value } pairs and resolving with the chosen values; destinyPickCards is now a four-line slot wrapper over it, so Healing Tree's call site is untouched and the two selections can't drift. It also gained `max`, which is what 'up to 2' actually needs: at the cap the UNCHOSEN tiles dim and stop responding (.at-limit), so the limit shows on the board instead of only being enforced when you press confirm — a picker that silently swallows your third click reads as broken. (2) Reversal already owned, inline, the fiddly 'pull a card out of a pile and repaint the slot' move — the pile's top card IS its face, so removing anything has to redraw it or empty the slot. That is now removeFromStackPile(pileEl, indices, emptyLabel), splicing high index first so the earlier ones stay valid and handing the cards back in pile order; Reversal calls it for one card and Voider for up to N. (A third copy of the same logic still lives in the grab/drop path around the stack-slot click handler; left alone deliberately — it pops the top rather than an arbitrary index, and rewiring the drag core is not this card's job.) JUDGMENT CALLS. (1) HISTORY ONLY, exactly as printed — not the Hand, not the Future, and not Atlantica-parked cards, which are a resource rather than a pile. (2) Naming the voided cards in the resolution leaks nothing: the Abyss is a public view-only zone any player can click and list, so those cards are about to be on show anyway. (3) The cap is per player, not a pool — 'each Player may send up to 2' is 2 each. COMPUTER: voiding is thinning, so the only question is 'which card do I least want to draw again', and the answer is always Steam — it never voids a Creature, Landmark, Artifact or Spark, because those ARE the payoff a thinner deck exists to reach. It also keeps a floor of 4 Steam across ALL its zones (new aiSteamPoolCounts, counting Hand + parked + History + Future), since a deck thinned past its economy can't buy the things that were supposed to justify it. Easy doesn't plan a deck and declines; Normal drops FireSteam, the free starter currency; Hard also drops GoldSteam ONCE it owns a LaserSteam — at that point the Gold is the weak card in that particular deck, which is exactly the judgment the card asks for. A REAL BUG worth recording, because the codebase has been bitten by it before (the duplicate gainTimePoints caught during the first Destiny commit): the pool counter was first written as aiSteamCounts(pNum) — and a function of that name ALREADY existed down in the Computer section, counting the HAND only and keying its result F/G/L. Two same-named function declarations in this one big scope don't collide loudly; the later one silently wins for the whole file. So the new code read {F,G,L}, summed three undefineds to NaN, and slice(0, NaN) returned an empty array — the Computer declined every Voider and looked merely cautious rather than broken. Renamed to aiSteamPoolCounts with a comment on both halves of the distinction. Verified live in BOTH modes. Vs Computer: the reveal fired, the picker listed Player 1's six History cards, selecting two dimmed the other four and a THIRD click was correctly ignored (still 2 selected), VOID 2 moved exactly the chosen indices out (History 6 → 4, the untouched cards intact and in order) and both landed in the shared Abyss, whose badge read '4 IN ABYSS' and whose list modal showed them. The Computer then voided its two FireSteam and kept its GoldSteam and Cravus. EDGE CASES: voiding a two-card pile down to nothing empties the slot properly (slot-empty, 'History' label restored, dataset cleared, no ghost art); an empty History Pile gives a CONTINUE notice instead of opening an empty picker. DIFFICULTY verified all three ways — Easy declined; Normal with a Laser owned and a Gold-only History still voided NOTHING (the Gold clause is Hard-only); Hard in the same spot voided both Golds. REGRESSION: Reversal after the extraction still lists its History, takes the chosen card to Hand and repaints the pile correctly. Hot seat: each seat saw only its OWN History, the device passed between them (PASS DEVICE / MY CHOICE) and handed back to the revealer (RESUME TURN) before the turn resumed at the Steam Phase. No console errors in any run. Destiny progress panel now reads 7/24, next 031 Space Voider." },
        ...
        function applyState() {
            ...
            if (sim.p2landmarks || sim.p2hand || sim.p2creatures || sim.p2future || sim.p2history ||
                ...
                setupBoard(2, {
                    ...
                    // Destiny's "Each Player…" cards act on both seats' piles, so a preset has
                    // to be able to stock the opponent's History as well as their Hand.
    ...
    // Pull cards out of a stack pile (History/Future) by index and repaint the slot — the
    // pile's top card IS its face, so removing anything has to redraw it, or empty the slot
    // when the last card goes. Returns the removed cards in the order they sat in the pile.
    // Shared by Reversal (one card, to Hand) and Voider (up to N, to the Abyss) so the
    // repaint rules can't drift between them.
    function removeFromStackPile(pileEl, indices, emptyLabel = 'History') {
        if (!pileEl) return [];
        let cards = [];
        try { cards = JSON.parse(pileEl.dataset.cardData || '[]'); } catch (e) { return []; }

        // Splice highest index first so the earlier ones stay valid, then unshift to hand
        // them back in pile order rather than the order they were removed.
        const wanted = [...new Set(indices)].filter(i => i >= 0 && i < cards.length).sort((a, b) => b - a);
        const removed = [];
        wanted.forEach(i => removed.unshift(cards.splice(i, 1)[0]));
        if (!removed.length) return [];

        if (cards.length === 0) {
            clearStackSlot(pileEl, emptyLabel);
        } else {
            pileEl.dataset.cardData = JSON.stringify(cards);
            const newTop = cards[cards.length - 1];
            const art = cardArtUrl(newTop);
            if (art) {
                pileEl.style.backgroundImage = `url('${art}')`;
            } else {
                pileEl.style.backgroundImage = '';
                pileEl.style.backgroundColor = 'rgba(255,255,255,0.1)';
                pileEl.textContent = newTop.name;
            }
            bindHoverToElement(pileEl, newTop);
            updateStackIndicator(pileEl);
        }
        return removed;
    }
    ...
    function resolveReversal(pNum) {
        ...
        const takeCardAt = (idx) => {
            const [taken] = removeFromStackPile(historyPile, [idx], 'History');
            if (!taken) return;
    ...
    // Multi-select over a set of cards, rendered as mini cards in the overlay. `items` are
    // { card, value } pairs and it resolves with the chosen VALUES ([] when the player
    // declines), in the order they were given. One picker therefore serves both a selection
    // over Hand SLOTS (Healing Tree) and one over the plain card records inside a pile
    // (Voider), instead of two copies of it drifting apart.
    //
    // `max` caps the selection — Voider's "up to 2". Once the cap is reached the unchosen
    // tiles dim and stop responding, so the limit shows on the board rather than only being
    // enforced when you press confirm. Omit it for an uncapped pick.
    function destinyPickTiles(items, { title, confirmLabel, declineLabel, live, max }) {
        ...
        return new Promise(resolve => {
            ...
            const refresh = () => {
                ...
                const full = max !== undefined && selected.size >= max;
                tiles.forEach(t => t.tile.classList.toggle('at-limit', full && !selected.has(t.value)));
            };
            ...
            items.forEach(item => {
                ...
                tile.onclick = (ev) => {
                    ev.stopPropagation();
                    if (selected.has(item.value)) { selected.delete(item.value); tile.classList.remove('selected'); }
                    else {
                        if (max !== undefined && selected.size >= max) return; // at the cap — say nothing, the tile is already dimmed
                        selected.add(item.value); tile.classList.add('selected');
                    }
                    refresh();
                };
                tiles.push({ value: item.value, tile });
            ...
            confirmBtn.onclick = (ev) => {
                ev.stopPropagation();
                const out = items.filter(i => selected.has(i.value)).map(i => i.value);
                picker.innerHTML = '';
                controls.innerHTML = '';
                setDestinyPrompt('');
                resolve(out);
    ...
    // Slot flavour of the picker: select over Hand slots, resolving with the slots themselves.
    ...
    // 030 Voider — "Each Player may send up to 2 Cards from their History Pile into the Abyss."
    // The first Destiny card that is pure DECK-BUILDING rather than combat or tempo: the
    // History Pile folds back into the Future when it runs dry, so a card sent to the Abyss
    // leaves that player's deck for good and every remaining draw gets better. Clockwise from
    // the revealer, and the "may" is real — a seat can void 2, 1 or nothing.
    //
    // Written as runVoider(max, cardName) because 031 Space Voider is this card with 4 instead
    // of 2 — one table row and one line, not a second copy. cardName is only for the Computer's
    // feed, so the log names the event that actually fired rather than always saying "Voider".
    //
    // JUDGMENT CALLS. (1) HISTORY ONLY, exactly as printed — not the Hand, not the Future, and
    // not Atlantica-parked cards, which are a resource rather than a pile. (2) Naming the
    // voided cards in the resolution leaks nothing: the Abyss is a public, view-only zone that
    // any player can click and list, so those cards are about to be on show anyway. (3) The
    // cap is per player, not a pool — "each Player may send up to 2" is 2 each.
    async function runVoider(revealer, max, cardName) {
        await forEachPlayerClockwise(revealer, async (pNum) => {
            const board = document.getElementById(`player-${pNum}`);
            if (!board) return;
            const isAI = vsComputer && pNum === AI_PLAYER;
            ...
            let history = [];
            try { history = JSON.parse((historyPile && historyPile.dataset.cardData) || '[]'); } catch (e) { history = []; }
            if (history.length === 0) {
                await destinyNotice(`${seatName}'s History Pile is empty — there is nothing to void.`);
                return;
            }

            const cap = Math.min(max, history.length);
            const picked = isAI
                ? aiVoiderPick(history, cap)
                : await destinyPickTiles(history.map((c, i) => ({ card: c, value: i })), {
                    max: cap,
                    live: (n) => n === 0
                        ? `Player ${pNum}: choose up to ${cap} card${cap === 1 ? '' : 's'} to remove from your deck for good — or decline.`
                        : `Voiding ${n} of ${cap} — these never come back, and every later draw improves.`,
                    confirmLabel: (n) => n === 0 ? 'VOID NOTHING' : `VOID ${n}`,
                    declineLabel: 'DECLINE',
                });

            if (picked.length === 0) {
                if (isAI) aiLog(`${cardName}: keeps its History Pile intact`, 'info');
                await destinyNotice(`${seatName} voids nothing.`);
                return;
            }

            const removed = removeFromStackPile(historyPile, picked, 'History');
            const abyssEl = document.querySelector('.card--abyss');
            removed.forEach(c => finishSingleCardPlacement(abyssEl, c));
            if (abyssEl) floatValue(abyssEl, `${removed.length} Voided`, 'damage');
            floatValue(historyPile, `-${removed.length}`, 'damage');

            const names = removed.map(c => c.name).join(', ');
            if (isAI) aiLog(`${cardName}: sends ${names} into the Abyss`, 'play');
            await destinyNotice(`${seatName} sends ${names} into the Abyss — out of the game for good.`);
        });
    }

    async function resolveVoider(card, revealer) {
        await runVoider(revealer, 2, card.name);
    }

    // 031 Space Voider — "Each Player may send up to 4 Cards from their History Pile into the
    // Abyss." Voider at double the depth and nothing else, which is why 030 was written as
    // runVoider(max): the rules, the picker cap, the Abyss handoff and the Computer's whole
    // heuristic are shared, and the difference between the two cards is this number.
    //
    // The cap still can't exceed the pile (runVoider clamps it), so on a thin History this
    // plays exactly like Voider — the extra reach only shows once a deck has cycled a few
    // times, which is precisely when thinning is worth the most.
    async function resolveSpaceVoider(card, revealer) {
        await runVoider(revealer, 4, card.name);
    }

    // The Computer's Voider call. Voiding is deck-thinning, so the question is only ever
    // "which card do I least want to draw again" — and the answer is always Steam. Creatures,
    // Landmarks, Artifacts and Sparks ARE the payoff a thinner deck exists to reach, so it
    // never voids one, however cheap. It also keeps a floor of Steam across all its zones:
    // a deck thinned past its economy can't buy the things that were supposed to justify it.
    //
    // Easy doesn't plan a deck at all and declines. Normal drops FireSteam, the free starter
    // currency. Hard also drops GoldSteam ONCE it owns a LaserSteam — at that point the Gold
    // is the weak card in that particular deck, which is the whole judgment the card asks for.
    const AI_STEAM_FLOOR = 4;
    function aiVoiderPick(history, cap) {
        if (aiLevel === 'easy') return [];

        const owned = aiSteamPoolCounts(AI_PLAYER);
        const total = owned.STM1 + owned.STM2 + owned.STM3;
        const trashable = (c) => c.number === 'STM1' ||
            (aiLevel === 'hard' && owned.STM3 > 0 && c.number === 'STM2');

        const spare = Math.max(0, total - AI_STEAM_FLOOR);
        if (spare === 0) return [];

        // Cheapest first, so a Hard player drops its Fires before its Golds.
        return history
            .map((c, i) => ({ i, c }))
            .filter(e => trashable(e.c))
            .sort((a, b) => cardCostValue(a.c) - cardCostValue(b.c))
            .slice(0, Math.min(cap, spare))
            .map(e => e.i)
            .sort((a, b) => a - b);
    }

    // Every Steam card a player owns, wherever it currently sits — Hand (parked cards
    // included, they spend like Hand cards), History and Future. That whole POOL is what a
    // thinning decision has to be measured against, not just the pile being thinned.
    //
    // Deliberately NOT named aiSteamCounts: that one already exists down in the Computer
    // section, counts the HAND only, and keys its result F/G/L. Two same-named function
    // declarations in this one scope don't collide loudly — the later one silently wins for
    // the whole file — so the two have to stay tellably different by name.
    function aiSteamPoolCounts(pNum) {
        const board = document.getElementById(`player-${pNum}`);
        const out = { STM1: 0, STM2: 0, STM3: 0 };
        if (!board) return out;
        const tally = (c) => { if (c && out[c.number] !== undefined) out[c.number]++; };
        board.querySelectorAll('.hand-slot:not(.slot-empty), .atlantica-slot:not(.slot-empty)')
            .forEach(sl => { try { tally(JSON.parse(sl.dataset.cardData)); } catch (e) { /* blank */ } });
        board.querySelectorAll('.history-pile, .future-pile').forEach(pile => {
            let cards = [];
            try { cards = JSON.parse(pile.dataset.cardData || '[]'); } catch (e) { cards = []; }
            if (Array.isArray(cards)) cards.forEach(tally);
        });
        return out;
    }

    // 032 Eternal Hour — "All Players tied for most Time Points lose 4."
    // The first Destiny card with no choice in it at all, and the first that is completely
    // IMPARTIAL: nothing about it keys off the revealer. Flip it while behind and it knocks
    // the leader down for you; flip it while ahead and it hits you and nobody else. That
    // asymmetry is the card — it is the game's catch-up mechanism, not a weapon.
    //
    // THE ONE THING THAT MATTERS IN THE CODE: the tied set is snapshotted BEFORE any damage
    // lands. Resolving seat by seat would let the first hit drop that player out of the lead
    // and change who is "tied for most" for everyone after them — so a 24/24 board would
    // punish only the first seat instead of both. "All Players tied for most" is judged once,
    // against the board as the card found it, then applied.
    //
    // JUDGMENT CALLS. (1) Only players still IN the game are considered: a seat on 0 Time
    // Points is out, and a corpse can't be tied for the lead. (In 2-player V1 this can't
    // arise — the game ends the moment a seat hits 0 — but the rule is written as it should
    // read at 4 players.) (2) It CAN end the game, and if both leaders are on 4 or less it
    // ends in a genuine draw; checkGameOver already handles that, so the notice is skipped
    // and the game-over screen takes over rather than making someone dismiss a card first.
    // (3) The TP system is untouched — this calls the same resolveDamageDirectly every attack
    // uses, so the active-die-first rule, the permanent loss of a drained die and the
    // game-over check all come along for free.
    const ETERNAL_HOUR_LOSS = 4;
    async function resolveEternalHour(card, revealer) {
        // Snapshot first: who is alive, what they hold, and which die is about to take it.
        const seats = seatsClockwise(revealer)
            .map(pNum => ({ pNum, tp: totalTimePoints(pNum) }))
            .filter(s => s.tp > 0);
        if (seats.length === 0) return;

        const most = Math.max(...seats.map(s => s.tp));
        const tied = seats.filter(s => s.tp === most);
        const seatName = (pNum) => (vsComputer && pNum === AI_PLAYER) ? 'The Computer' : `Player ${pNum}`;

        tied.forEach(({ pNum }) => {
            const board = document.getElementById(`player-${pNum}`);
            // Read the die BEFORE the hit — draining it can change which one is active.
            const dieEl = board && board.querySelector(activeDieSel(pNum));
            resolveDamageDirectly(ETERNAL_HOUR_LOSS, pNum);
            if (dieEl) floatValue(dieEl, `-${ETERNAL_HOUR_LOSS} TP`, 'damage');
        });

        const names = tied.map(t => seatName(t.pNum)).join(' and ');
        // "The Computer" and "Player 2" are singular subjects, so the verbs have to agree —
        // one leader leads and loses, several lead and lose.
        const one = tied.length === 1;
        if (vsComputer) aiLog(`Eternal Hour: ${names} lose${one ? 's' : ''} ${ETERNAL_HOUR_LOSS} Time Points`, 'combat');

        // A draw or a win here is the whole story — don't make anyone dismiss a card to see it.
        if (gameWon) return;

        // "Every player" only reads right when there is more than one of them; with a single
        // survivor left standing it is just that player leading.
        await destinyNotice(tied.length === seats.length && seats.length > 1
            ? `Every player is level on ${most} Time Points — all of them lose ${ETERNAL_HOUR_LOSS}.`
            : `${names} lead${one ? 's' : ''} on ${most} Time Points and lose${one ? 's' : ''} ${ETERNAL_HOUR_LOSS}.`);
    }
    ...
    (function initMobileMode() {
        ...
        // The mobile "camera" puts a CSS transform on #game-field, which makes it
        // the containing block for every position:fixed descendant — so any modal
        // or overlay left inside the field is sized/positioned against the scaled
        // board box instead of the viewport, collapsing into a tiny, unusable
        // sliver in the corner (this is what made the menu tools & Database look
        // like "nothing happens" on a phone). #card-modal was already pulled out
        // for this reason; relocate the remaining full-screen panels & combat
        // overlays to <body> so their fixed positioning resolves against the
        // viewport. Harmless on desktop (no transform there — fixed is
        // viewport-relative regardless of DOM location), and all of them are
        // reached via document.getElementById, so no lookup breaks.
        [
            'database-screen', 'devlog-screen',
            'keywords-list-modal', 'rules-modal', 'keyword-modal',
