...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const simulationMap = {
        ...
        'Space Voider': {
            phase: 0,
            // Six cards in each History Pile, so the 4-card allowance is a real ceiling to
            // bump into rather than "select everything". Two cards in each pile are worth
            // keeping (a Creature and a LaserSteam), which is what makes it a choice.
            desc: "Player 1's Future Pile is empty — the turn start reveals Space Voider — Voider's big brother: up to 4 instead of 2. Your six History cards fan out; a FIFTH pick should not take until you deselect one. The Computer's own pile is stocked too (Normal voids 2 of its cheapest Steam, Hard 4, Easy declines) — and it stops early rather than thin its deck below 6 cards.",
            destiny: 'Space Voider',
            hand: ['FireSteam', 'GoldSteam'],
            p1future: [],
            p1history: ['FireSteam', 'FireSteam', 'FireSteam', 'GoldSteam', 'LaserSteam', 'Ichor'],
            p2future: ['FireSteam', 'GoldSteam', 'LaserSteam', 'Ichor'],
    ...
    // 031 Space Voider — the same card with 4 instead of 2. Everything that made Voider
    // work is in voidFromHistory; the only thing this card is, is a bigger number.
    async function resolveSpaceVoider(card, revealer) {
        await voidFromHistory(revealer, 4, 'Space Voider');
    }
    ...
    function aiVoiderPick(board, history, limit) {
        // Scaled to the card rather than fixed, so Space Voider's bigger allowance actually
        // means something to the Computer: Hard takes the lot, Normal takes half (1 of 2, 2 of 4).
        const cap = aiLevel === 'hard' ? limit : aiLevel === 'normal' ? Math.ceil(limit / 2) : 0;
        if (cap === 0) return [];
